"""
Tests for veyra.server.

Regression cover for #656: `python -m veyra server` broke on Playwright
...
from veyra import server  # noqa: E402
from veyra.server import get_nodejs  # noqa: E402
...
def test_launch_script_resolves_driver_against_installed_playwright():
    ...
    result = subprocess.run(
        ...
        input=base64.b64encode(
            orjson.dumps({"executablePath": "/nonexistent/veyra-bin"})
        ).decode(),
        capture_output=True,
        text=True,
        timeout=120,
