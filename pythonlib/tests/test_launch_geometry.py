...
from veyra import utils  # noqa: E402
...
def host(screen_cons):
    ...
    ), mock.patch.object(utils, "launch_path", lambda **kwargs: "/nonexistent/veyra"):
        yield


def config_of(options):
    """Reassemble the chunked VEYRA_CONFIG_<n> env vars into a dict."""
    env = options["env"]
    chunks = sorted(
        (int(k.rsplit("_", 1)[1]), v) for k, v in env.items() if k.startswith("VEYRA_CONFIG_")
    )
