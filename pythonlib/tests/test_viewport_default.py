...
from veyra.utils import attach_no_viewport_default, spoofs_window_dimensions
...
def _opts(config_blob: str):
    ...
    return {"env": {f"VEYRA_CONFIG_{i + 1}": c for i, c in enumerate(chunks)}}
...
def test_reassembles_chunks_in_index_order():
    """VEYRA_CONFIG_10 must not sort before VEYRA_CONFIG_2 -- a lexicographic
