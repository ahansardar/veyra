...
from veyra import multiversion, pkgman, utils
...
def test_missing_profile_directory_in_read_only_home_fails_fast(tmp_path, monkeypatch):
    ...
    home.chmod(0o500)
    try:
        with pytest.raises(RuntimeError, match=r"\.veyra.*before.*read-only"):
...
def test_fetch_prepares_profile_directory(monkeypatch):
    ...
    fetcher = object.__new__(pkgman.VeyraFetcher)
