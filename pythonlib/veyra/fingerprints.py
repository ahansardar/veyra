...
from veyra.pkgman import load_yaml
from veyra.webgl import sample_webgl
...
def from_preset(preset: Dict, ff_version: Optional[str] = None) -> Dict[str, Any]:
    """
    Convert a real fingerprint preset to VEYRA_CONFIG format.
    """
    config: Dict[str, Any] = {}
...
def _build_init_script(values: Dict[str, Any]) -> str:
    ...
    # Without this, the C++ MaskConfig fallback (from VEYRA_CONFIG set by geoip
...
def generate_context_fingerprint(
    ...
    Parameters:
        ...
        config_overrides: Dict of VEYRA_CONFIG keys to override after config
...
def handle_screenXY(veyra_data: Dict[str, Any], fp_screen: ScreenFingerprint) -> None:
    ...
    if screenY == 0:
        ...
        veyra_data['window.screenY'] = randrange(0, screenY)  # nosec
    else:
        veyra_data['window.screenY'] = randrange(screenY, 0)  # nosec


def from_browserforge(fingerprint: Fingerprint, ff_version: Optional[str] = None) -> Dict[str, Any]:
    """
    Converts a Browserforge fingerprint to a Veyra config.
    """
    veyra_data: Dict[str, Any] = {}
    _cast_to_properties(
        veyra_data,
    ...
    handle_screenXY(veyra_data, fingerprint.screen)
