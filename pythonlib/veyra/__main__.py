...
def _ensure_synced() -> bool:
    ...
    if not REPO_CACHE_FILE.exists():
        rprint("No repo cache found. Run 'veyra sync' first.", fg="red")
        return False
    return True


class VeyraUpdate(VeyraFetcher):
    """
    Checks & updates Veyra
    """
    ...
    def update(self, replace: bool = False, i_know_what_im_doing: bool = False) -> None:
        if not self.is_updated_needed() and not replace:
            rprint("Veyra binaries up to date!", fg="green")
        ...
        rprint(f"{action} Veyra v{self.verstr}...", fg="yellow")
...
def fetch(version):
    ...
    \b
    Examples:
      veyra fetch                         # install active version
      veyra fetch official/135.0-beta.25  # install specific version
    """
    ...
    if ver_data is None:
        rprint(f"Version '{version or repo_name}' not found in cache. Run 'veyra sync'.", fg="red")
        return
    ...
    try:
        VeyraUpdate(repo_config=repo_config, selected_version=selected).update()
    except Exception as e:
        msg = str(e)
        if "404" in msg or "Not Found" in msg:
            rprint("Release not found (404). Asset may have been removed.", fg="red")
            rprint("Run 'veyra sync' to refresh available versions.", fg="yellow")
        else:
...
def _set_channel(repo_name: str, channel_type: str):
    ...
    click.secho("Run 'veyra fetch' to install latest.", fg="yellow")
...
def _set_pinned(repo_name: str, channel_type: str, ver_data: dict, inst, sha: Optional[str] = None):
    ...
    if inst:
        ...
        click.secho("Run 'veyra fetch' to install.", fg="yellow")
...
def set_cmd(specifier, geoip):
    """
    \b
    Set the active Veyra version to use & fetch.
    ...
    if specifier:
        ...
        if repo_data is None:
            rprint(f"Repo '{repo_name.lower()}' not in cache. Run 'veyra sync'.", fg="red")
            return
    ...
    if not cache.get("repos"):
        rprint("No versions in cache. Run 'veyra sync' first.", fg="red")
        return
...
def remove(version_path, select, yes):
    ...
    if yes or click.confirm(f"Remove the veyra data directory ({INSTALL_DIR})?"):
        shutil.rmtree(INSTALL_DIR)
        rprint("Removed veyra data directory.", fg="green")


@cli.command(name="test")
@click.option("--executable-path", help="Path to the Veyra executable", default=None)
...
def gui(debug):
    """
    Launch the Veyra Manager GUI (requires PySide6)
    """
    try:
        ...
        rprint(
            "GUI requires PySide6. Install with: pip install 'veyra\\[gui]'",
            fg="red",
        )
...
class VersionInfo:
    ...
    def geoip(self):
        ...
        if not ALLOW_GEOIP:
            ...
            self._row("Status", "Not supported (install veyra[geoip])", style="dim")
        else:
