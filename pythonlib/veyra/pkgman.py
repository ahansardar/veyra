...
INSTALL_DIR: Path = Path(user_cache_dir("veyra"))
...
def ensure_browser_profile_dir(
    ...
    Firefox probes ``~/.veyra`` even when Playwright supplies a temporary
    ...
    try:
        ...
        raise ProfileDirectoryError(
            f"Veyra requires '{profile_dir}' to exist before launch, but it "
    ...
    if not profile_dir.is_dir():
        raise ProfileDirectoryError(
            f"Veyra requires '{profile_dir}' to be a directory before launch."
        )
    return profile_dir
...
class RepoConfig:
    """
    Configuration for a Veyra repository
    """
    ...
    def from_path(path: Optional[Path] = None) -> 'Version':
        ...
        if not os.path.exists(version_path):
            raise FileNotFoundError(
                ...
                "Please run `veyra fetch` to install."
            )
...
class AvailableVersion:
    """
    Information about an available Veyra version from GitHub
    """
...
class VeyraFetcher(GitHubDownloader):
    """
    Handles fetching and installing Veyra
    """
    ...
    def fetch_latest(self) -> None:
        """
        Fetch the latest veyra release for the current platform
        """
    ...
    def extract_zip(self, zip_file: DownloadBuffer) -> None:
        ...
        """
        rprint(f'Extracting Veyra: {INSTALL_DIR}')
    ...
    def install(self, replace: bool = False) -> None:
        """
        Download and install veyra to a versioned subdirectory
        """
...
def installed_verstr() -> str:
    ...
    if active is None:
        ...
        raise VeyraNotInstalled(
            f"{active_display} is not installed. " f"Please run `veyra fetch` to install."
        )
...
def veyra_path(download_if_missing: bool = True) -> Path:
    """
    Full path to the active veyra folder
    """
    ...
    if not os.path.exists(INSTALL_DIR) or not os.listdir(INSTALL_DIR):
        ...
        if not download_if_missing:
            raise UnsupportedVersion("Veyra executable is outdated.")

    VeyraFetcher().install()
    ...
    raise UnsupportedVersion(
        f"No available Veyra build satisfies this library's minimum "
        f"({CONSTRAINTS.MIN_VERSION}). The matching browser release may not be "
        f"published yet; wait for it, or install an older veyra release."
    )


def get_path(file: str) -> str:
    """
    Get the path to a file in the veyra directory
    """
    if OS_NAME == 'mac':
        return os.path.abspath(veyra_path() / 'Veyra.app' / 'Contents' / 'Resources' / file)
    return str(veyra_path() / file)


def launch_path(browser_path: Optional[Path] = None) -> str:
    """
    Get the path to the veyra executable
    """
    if browser_path:
        if OS_NAME == 'mac':
            exec_path = os.path.abspath(
                browser_path / 'Veyra.app' / 'Contents' / 'Resources' / LAUNCH_FILE[OS_NAME]
            )
        else:
    ...
    if not os.path.exists(exec_path):
        raise VeyraNotInstalled(
            f"Veyra is not installed at {browser_path or veyra_path()}. Please run `veyra fetch` to install."
        )
    return exec_path
