...
# Veyra preferences to cache previous pages and requests
CACHE_PREFS = {
...
def _generate_fontconfig(fontconfig_path: str, path: Optional[Path] = None) -> str:
    ...
    # INSTALL_DIR is platformdirs' user_cache_dir("veyra"); see pkgman.
...
def warn_if_executable_predates_playwright(path: Optional[Path]) -> None:
    ...
    Veyra defaults to no_viewport when it spoofs window dimensions
    ...
    warnings.warn(
        f"The Veyra build at {path} is {installed.build}, but Playwright "
...
def get_env_vars(
    ...
    """
    Gets a dictionary of environment variables for Veyra.
    ...
    for i in range(0, len(config_str), chunk_size):
        ...
        env_name = f"VEYRA_CONFIG_{(i // chunk_size) + 1}"
        try:
    ...
    if OS_NAME == 'lin':
        ...
        if not os.path.exists(os.path.join(fontconfig_path, "fonts.conf")):
            ...
            raise FileNotFoundError(
                f"fonts.conf not found in {fontconfig_path}!  Something ain't right with your veyra bundle."
            )
    ...
    for index, voice in enumerate(voices):
        if not isinstance(voice, dict):
            raise InvalidPropertyType(
                ...
                f"({voice!r}). Veyra needs full voice objects, not names."
            )
...
def get_screen_cons(headless: Optional[bool] = None) -> Optional[Screen]:
    """
    Determines a sane viewport size for Veyra if being ran in headful mode.

    Bounds are CSS pixels, the unit Firefox lays its windows out in -- see
    veyra.display for why that differs from the monitor's physical size.
    """
...
def check_custom_fingerprint(fingerprint: Fingerprint) -> None:
    ...
    if browser_name != 'Firefox':
        raise NonFirefoxFingerprint(
            f'"{browser_name}" fingerprints are not supported in Veyra. '
...
def check_valid_os(os: ListOrString) -> None:
    ...
    # Assert that the OS is supported by Veyra
    if os not in ('windows', 'macos', 'linux'):
        raise InvalidOS(f"Veyra does not support the OS: '{os}'")
...
def warn_manual_config(config: Dict[str, Any]) -> None:
    """
    Warns the user if they are manually setting properties that Veyra already sets internally.
    """
...
def spoofs_window_dimensions(from_options: Dict[str, Any]) -> bool:
    """
    Whether the VEYRA_CONFIG in a set of launch options spoofs any window
    dimension. The config is chunked across VEYRA_CONFIG_<n> env vars, so
    reassemble it in index order before looking.
    """
    env = from_options.get('env') or {}
    chunks = [(int(k.rsplit('_', 1)[1]), v) for k, v in env.items() if k.startswith('VEYRA_CONFIG_')]
    if not chunks:
        return False
...
def launch_options(
    ...
    **launch_options: Dict[str, Any],
    ...
    """
    Launches a new browser instance for Veyra.
    Accepts all Playwright Firefox launch options, along with the following:

    Parameters:
        config (Optional[Dict[str, Any]]):
            Veyra properties to use. (read https://github.com/ahansardar/veyra/blob/main/README.md)
        ...
        locale (Optional[Union[str, List[str]]]):
            Locale(s) to use in Veyra. The first listed locale will be used for the Intl API.
        addons (Optional[List[str]]):
            List of Firefox addons to use.
        fonts (Optional[List[str]]):
            Fonts to load into Veyra (in addition to the default fonts for the target `os`).
            Takes a list of font family names that are installed on the system.
        custom_fonts_only (Optional[bool]):
            If enabled, OS-specific system fonts will be not be passed to Veyra.
        exclude_addons (Optional[List[DefaultAddons]]):
            Default addons to exclude. Passed as a list of veyra.DefaultAddons enums.
        ...
        ff_version (Optional[int]):
            Firefox version to use. Defaults to the current Veyra version.
            To prevent leaks, only use this for special cases.
        headless (Optional[bool]):
            Whether to run the browser in headless mode. Defaults to False.
            Note: If you are running linux, passing headless='virtual' to Veyra & AsyncVeyra
            will use Xvfb.
        ...
        executable_path (Optional[Union[str, Path]]):
            Custom Veyra browser executable path.
        ...
        debug (Optional[bool]):
            Prints the config being sent to Veyra.
        virtual_display (Optional[str]):
            Virtual display number. Ex: ':99'. This is handled by Veyra & AsyncVeyra.
    ...
    # entry. settings/veyra.cfg now runs Firefox's stock max_entries, so the
    ...
    # only stops it when Veyra owns the list. Leaving `voices` unset -- which
    ...
    # Prepare environment variables to pass to Veyra
    env_vars = {
    ...
    if executable_path:
        ...
        if not browser_path:
            raise ValueError(
                f"Browser version '{browser}' not found. Run `veyra list` to see installed versions."
            )
