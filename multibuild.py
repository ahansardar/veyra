...
Since Veyra is NOT meant to be used as a daily driver, no installers are provided.
"""
...
class BSYS:
    ...
    def build(self):
        """Build the Veyra source code"""
        os.environ['BUILD_TARGET'] = f'{self.target},{self.arch}'
        run('make build')

    def package(self):
        """Package the Veyra source code"""
    ...
    def assets(self) -> List[str]:
        ...
        package_pattern = f'veyra-*-{self.target[:3]}.{self.arch}.zip'
        return glob.glob(package_pattern)

    @staticmethod
    def clean():
        """Clean the Veyra directory"""
        run('make clean')
...
def main():
    parser = argparse.ArgumentParser(description="Easy build CLI for Veyra")
    parser.add_argument(
        "--target",
