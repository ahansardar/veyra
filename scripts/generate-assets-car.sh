#!/bin/bash
# Generates Assets.car for macOS builds from the veyra branding icons
# This script must be run on macOS (requires actool from Xcode)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BRANDING_DIR="$SCRIPT_DIR/../additions/browser/branding/veyra"
...
if [[ "$(uname)" != "Darwin" ]]; then
    ...
    if [[ ! -f "$OUTPUT_FILE" ]]; then
        OFFICIAL_ASSETS="$SCRIPT_DIR/../veyra-*/browser/branding/official/Assets.car"
...
echo "Generating Assets.car from veyra branding icons..."
