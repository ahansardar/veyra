...
"""
GUI for managing Veyra patches.
"""
import os
import re
import sys
...
def handle_choice(choice):
    ...
    match choice:
        ...
        case "Create new patch":
            # Reset veyra, apply all patches, then create a checkpoint
            reset_veyra()
            with temp_cd('..'):
                run('make dir')
                run('make first-checkpoint')
            easygui.msgbox(
                "Created new patch workspace. You can test Veyra with 'make run'.\n\n"
...
if __name__ == "__main__":
    ...
    while choice := easygui.choicebox("Select an option:", "Veyra Dev Tools", choices):
