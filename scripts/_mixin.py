...
"""
Common functions used across the Veyra build system.
...
def find_src_dir(root_dir='.', version=None, release=None):
    ...
    if version and release:
        name = os.path.join(root_dir, f'veyra-{version}-{release}')
        assert os.path.exists(name), f'{name} does not exist.'
        return name
    folders = os.listdir(root_dir)
    for folder in folders:
        if os.path.isdir(folder) and folder.startswith('veyra-'):
            return os.path.join(root_dir, folder)
    raise FileNotFoundError('No veyra-* folder found')
