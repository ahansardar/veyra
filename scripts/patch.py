...
"""
The script that patches the Firefox source into the Veyra source.
...
@dataclass
class Patcher:
    """Patch and prepare the Veyra source"""

    moz_target: str
    target: str

    def veyra_patches(self):
        """
        Apply all patches
        """
...
def extract_args():
    ...
    if len(args) != 2:
        sys.stderr.write('error: please specify version and release of veyra source')
        sys.exit(1)
