#!/usr/bin/env bash
# Install a custom Veyra build into the local channel.
#
# Usage:
...
#
# Installs to ~/.cache/veyra/browsers/local/<version-build>/
...
CACHE_DIR="${HOME}/Library/Caches/veyra"
# Fall back to XDG if not on macOS
if [[ ! -d "${HOME}/Library/Caches" ]]; then
    CACHE_DIR="${XDG_CACHE_HOME:-${HOME}/.cache}/veyra"
fi
...
ARTIFACT="${1:-}"
if [[ -z "$ARTIFACT" ]]; then
    ARTIFACT="$(ls -t "$REPO_ROOT"/dist/veyra-*-mac.arm64.zip 2>/dev/null | head -1)"
    if [[ -z "$ARTIFACT" ]]; then
...
if [[ -z "$VERSION_BUILD" ]]; then
    # Extract from filename: veyra-<version>-<build>-mac.arm64.zip
    BASENAME="$(basename "$ARTIFACT")"
    # Strip prefix "veyra-" and suffix "-mac.arm64.zip" (or similar)
    VERSION_BUILD="${BASENAME#veyra-}"
...
# Handle macOS structure: the zip may contain Veyra.app directly or nested
if [[ -d "$TMP_DIR/Veyra.app" ]]; then
    mv "$TMP_DIR/Veyra.app" "$INSTALL_DIR/Veyra.app"
elif [[ -d "$TMP_DIR/Veyra/Veyra.app" ]]; then
    mv "$TMP_DIR/Veyra/Veyra.app" "$INSTALL_DIR/Veyra.app"
else
...
# Verify
PLIST="$INSTALL_DIR/Veyra.app/Contents/Info.plist"
if [[ -f "$PLIST" ]]; then
...
echo ""
echo "Done. Run 'veyra list' to see installed versions."
