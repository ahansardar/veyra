...
from veyra.pkgman import launch_path
from veyra.sync_api import Veyra
from veyra.virtdisplay import VirtualDisplay
...
def run_playwright(mode, browser_name):
    ...
    if browser_name == "veyra-ubo":
        veyra = Veyra(headless=headless, env=env)
        browser = veyra.start()
    elif browser_name == "firefox":
        playwright = sync_playwright().start()
        browser = playwright.firefox.launch(headless=headless, env=env)
    elif browser_name == "veyra":
        playwright = sync_playwright().start()
    ...
    for url in urls:
        ...
        memory = get_average_memory(
            name="veyra-bin" if browser_name.startswith('veyra') else 'firefox', duration=10
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--browser",
        type=str,
        choices=["firefox", "veyra", "veyra-ubo"],
        required=True,
