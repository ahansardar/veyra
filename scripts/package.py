...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move Veyra/Veyra.app -> Veyra.app
            nightly_dir = os.path.join(temp_dir, 'Veyra')
            shutil.move(
                os.path.join(nightly_dir, 'Veyra.app'), os.path.join(temp_dir, 'Veyra.app')
            )
            # Remove old app dir and all content in it
            shutil.rmtree(nightly_dir)
        else:
            # Move contents out of veyra folder if it exists
            old_veyra_dir = os.path.join(temp_dir, 'veyra')
            veyra_dir = os.path.join(temp_dir, 'veyra-folder')
            if os.path.exists(old_veyra_dir):
                # Rename veyra_dir
                os.rename(old_veyra_dir, veyra_dir)
                for item in os.listdir(veyra_dir):
                    shutil.move(os.path.join(veyra_dir, item), temp_dir)
                os.rmdir(veyra_dir)

        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'Veyra.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Package Veyra for different operating systems.'
    )
    ...
    parser.add_argument('--version', required=True, help='Veyra version')
    parser.add_argument('--release', required=True, help='Veyra release number')
    parser.add_argument(
...
def main():
    ...
    with temp_cd(src_dir):
        ...
        search_path = os.path.abspath(
            f'obj-{moz_target}/dist/veyra-{args.version}-{args.release}.*.{file_ext}'
        )
    ...
    package_pattern = f'veyra-{args.version}-{args.release}.en-US.*.{file_ext}'
    ...
    new_name = f'veyra-{args.version}-{args.release}-{args.os[:3]}.{args.arch}.zip'
