...
Tests a raw Veyra binary (Firefox) directly against the same antibot-detection checks used in the service tests. Use this to validate a binary before packaging/releasing — it bypasses the Python package entirely.
...
**Example:**
```bash
python scripts/run_tests.py /path/to/veyra-bin/veyra
```

## Options

```
  binary_path           Path to the Veyra (Firefox) binary
...
**Global phase (2 profiles)** — 1 macOS + 1 Linux profile launched with fingerprint config passed via the `VEYRA_CONFIG` environment variable. Tests that browser-level fingerprint injection works correctly.
