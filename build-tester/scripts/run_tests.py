...
def main():
    parser = argparse.ArgumentParser(
        description="Veyra Build Tester — runs antibot-detection checks via Playwright",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("binary_path", help="Path to the Veyra (Firefox) binary")
    parser.add_argument(
        "--profile-count", type=int, default=8, metavar="N",
        help="Number of profiles to test, 1-8 (default: 8)",
    )
    parser.add_argument(
        "--secret", default="veyra-tester-dev-secret", metavar="KEY",
    ...
    if sys.platform == "darwin" and binary_path.endswith(".app"):
        candidate = os.path.join(binary_path, "Contents", "MacOS", "veyra")
