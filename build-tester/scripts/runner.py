"""
Playwright test runner — launches Veyra, runs per-context and global profiles,
...
_RESULTS_NODE = "__veyraResults__"
_READ_RESULTS = (
...
async def run_tests(
    ...
    print("\nGenerating fingerprint presets via Veyra Python API...")
    ...
    async with async_playwright() as pw:
        ...
        for entry in global_entries:
            ...
            browser = None
            try:
                env = {**dict(os.environ), "VEYRA_CONFIG": json.dumps(preset["veyraConfig"])}
                browser = await firefox.launch(
                    executable_path=binary_path,
                    headless=True,
                    env=env,
                ...
                # Inject only WebRTC IP for global profiles (VEYRA_CONFIG handles everything else)
