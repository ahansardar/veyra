...
> {
  ...
  result.crossSignal.webglRendererVsPlatform = (() => {
    try {
      ...
      // Check for "or similar" suffix from Veyra global config (expected behavior)
  ...
  // Note: Veyra applies per-profile audio transformations via getChannelData() hook,
  ...
  try {
    ...
    result.headlessDetection.hasTaskbar = (() => {
      ...
      // Veyra screen spoofing may set avail === screen
  ...
  try {
    ...
    if (claimedOS === "macos") {
      ...
      // Threshold of 3: Veyra bundles a subset of macOS fonts, not a full install.
