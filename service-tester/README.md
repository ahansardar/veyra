...
End-to-end antibot-detection tests that verify a pip-installed veyra release works correctly — both the Firefox binary and the Python package — using real proxies for each browser context.
...
4. **Phase 1** — run the suite against the locally compiled binary, auto-detected from:
   - macOS: `../veyra-*/obj-*-apple-darwin/dist/Veyra.app/Contents/MacOS/veyra`
   - Linux: `../veyra-*/obj-*-linux-*/dist/bin/veyra-bin`

   Skipped (with a notice) if no local build is found. On macOS, the script also copies `properties.json` from `Veyra.app/Contents/Resources/` into `MacOS/` so pythonlib can locate it next to the binary.
...
- A unique fingerprint generated by veyra via BrowserForge (navigator, screen, WebGL, fonts, voices, audio/canvas seeds)
- A distinct timezone
...
The cross-profile uniqueness section confirms each context has distinct audio, canvas, timezone, and screen fingerprints — verifying veyra generates genuinely different identities per context.

## Failure Triage

If a check fails, **fix it in the Python package** (`../pythonlib/veyra/`), not in the test. The test is intentionally a black-box validator — it only uses the public `AsyncNewContext` API and trusts veyra to produce correct fingerprints.
