...
echo "==> Building veyra wheel from ../pythonlib..."
$PIP install -q build
...
#  - macOS: Veyra.app/Contents/MacOS/veyra (bare bin/veyra-bin can't find dylibs)
#  - Linux: bin/veyra-bin
LOCAL_BIN=""
if [[ "$BINARY_MODE" != "fetched" ]]; then
    if [[ "$(uname -s)" == "Darwin" ]]; then
        LOCAL_GLOB=(../veyra-*/obj-*-apple-darwin/dist/Veyra.app/Contents/MacOS/veyra)
    else
        LOCAL_GLOB=(../veyra-*/obj-*-linux-*/dist/bin/veyra-bin)
    fi
