#!/usr/bin/env python3
"""
Veyra Service Tester — Python CLI

Tests an official Veyra release (installed via pip) using the same
antibot-detection checks as the build-tester, but launched via the
veyra Python API instead of a raw binary path.

Usage:
  python run_tests.py [options]

Options:
  --browser-version VER   Veyra version specifier (default: official/stable)
...
async def run_tests(
    ...
    # 3. Build profile specs — fingerprints and timezone resolved by veyra per-context
    all_specs = [
    ...
    try:
        from veyra.async_api import AsyncVeyra, AsyncNewContext
    except ImportError:
        print("ERROR: veyra package not installed.", file=sys.stderr)
        return 1
    ...
    try:
        async with AsyncVeyra(**launch_kwargs) as browser:
            # Create all contexts simultaneously — veyra handles all fingerprint injection
            open_contexts = []
        ...
        print(f"{RED}ERROR: Failed to launch Veyra: {e}{RESET}", file=sys.stderr)
        return 1
...
def main():
    parser = argparse.ArgumentParser(description="Veyra Service Tester")
    parser.add_argument("--browser-version", default="official/stable",
                        help="Veyra version specifier (default: official/stable)")
    ...
    parser.add_argument("--secret", default="veyra-service-test",
    ...
    parser.add_argument("--executable-path", default=None,
                        help="Use a specific Veyra binary (skips fetched-version selection)")
