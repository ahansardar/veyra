...
Veyra spoofs fingerprints globally via `VEYRA_CONFIG` — every browser context shares the same identity. These patches add **per-context isolation**, so each Playwright context can have a unique, deterministic fingerprint. This lets you run multiple concurrent sessions from a single Veyra process without cross-context correlation.
...
**Why `addInitScript`?** Each `window.setXxx()` function self-destructs after the first call. On new tabs or navigations, Veyra recreates the functions, and the init script calls them again before any page scripts can run. This ensures fingerprints are always applied and functions are never visible to website code.

**Why `typeof` guards?** The init script runs on every page load. If Veyra is ever used without these patches (e.g. vanilla build), the guards prevent `ReferenceError`. They also handle the case where the function was already called and self-destructed.
...
### Configuration (`settings/veyra.cfg`)

The `veyra.cfg` file sets Firefox preferences at startup (before `prefs.js` is loaded). Key settings:
...
**MaskConfig fallback:** If no per-context seed is set, checks `MaskConfig::GetUint32("audio:seed")` from `VEYRA_CONFIG`. This enables the Veyra Python package to set a global audio seed without per-context JavaScript.
...
**How it works:** Stores dimensions per context, then hooks `nsScreen::GetRect()` with a three-tier fallback: per-context values -> global `VEYRA_CONFIG` -> vanilla Firefox.
...
This replaces the old `screen-hijacker.patch` (which only supported global config). It includes the same global `VEYRA_CONFIG` fallback, so it works for both single-context and multi-context use cases.
...
**Controls:** `navigator.platform`, `navigator.oscpu`, `navigator.hardwareConcurrency`, `navigator.userAgent`, and `navigator.appVersion` — per-context with global `VEYRA_CONFIG` fallback.

**How it works:** Stores values per context via `NavigatorManager`, then hooks these Navigator methods with a three-tier fallback: per-context values -> global `VEYRA_CONFIG` -> vanilla Firefox:
...
**`setNavigatorUserAgent`:** Stores a per-context User-Agent string so workers report the correct UA. Without this, workers on Linux would read the global `VEYRA_CONFIG` UA (which may be a Linux UA) even when the per-context fingerprint specifies macOS. The WorkerNavigator hook checks `NavigatorManager` BEFORE `MaskConfig`, ensuring workers match the main page.
...
**Lazy timezone init:** Also adds `EnsureGlobalTimezoneInitialized()` — a lazy initializer that reads `timezone` from `VEYRA_CONFIG` on first access to `GetPlatform()` or `HardwareConcurrency()`. This replaced a static initializer that caused SIGSEGV crashes because SpiderMonkey wasn't ready at init time.
...
**How it works:** Stores vendor and renderer strings per context via `WebGLParamsManager`, then hooks `ClientWebGLContext::GetParameter()` to intercept `WEBGL_debug_renderer_info` queries. Three-tier fallback: per-context values -> global `VEYRA_CONFIG` -> real hardware values.
...
**Global MaskConfig scope:** Beyond per-context vendor/renderer, this patch also includes comprehensive global `VEYRA_CONFIG` spoofing for many other WebGL parameters — context attributes, shader precision, extensions, enabled states, and array parameters. These global overrides apply to all contexts equally and don't require per-context JavaScript.
...
**MaskConfig fallback:** If no per-context seed is set, checks `MaskConfig::GetUint32("canvas:seed")` from `VEYRA_CONFIG`.
...
These patches read from `VEYRA_CONFIG` at startup and apply to all contexts equally. They don't expose any `window.setXxx()` functions.
...
**`FONTCONFIG_PATH` environment variable:** Must be set when launching Veyra on Linux. Points to the correct OS-specific fontconfig directory (e.g. `veyra/fontconfig/macos/`). The Go launcher sets this dynamically based on the target OS.

---

## Python Library Changes

The Veyra Python package (`pythonlib/`) generates fingerprints for both `NewBrowser` (global VEYRA_CONFIG) and `NewContext` (per-context init script). **BrowserForge is the default for both paths.** Real fingerprint presets are available as an opt-in alternative.
...
async with AsyncVeyra(fingerprint_preset=True, os='macos') as browser:
    page = await browser.new_page()
    ...

# Or pin a specific preset dict
async with AsyncVeyra(fingerprint_preset=my_preset_dict) as browser:
    ...
```
...
|----------|--------|-------|
| UA, platform, HWC, oscpu | BrowserForge or preset | UA version patched to match Veyra Firefox version |
...
- `from_preset()` — converts real preset to VEYRA_CONFIG format
- `from_browserforge()` — converts BrowserForge Fingerprint to VEYRA_CONFIG using `browserforge.yml` mappings
...
- `launch_options()` — builds VEYRA_CONFIG env var, Playwright args, and Firefox prefs
...
**`properties.json`** — Includes `audio:seed` and `canvas:seed` as `VEYRA_CONFIG` properties (uint type). These enable the MaskConfig fallback in the audio and canvas patches when using global config without per-context JavaScript.

**`veyra.cfg`** — Sets `fission.autostart=true` and `dom.ipc.processPrelaunch.enabled=false`. No `dom.ipc.processCount` override needed with cross-process storage.
...
**Recommendation:** Always run Veyra on the OS that matches the fingerprint profile. Use macOS fingerprints on macOS workers, Linux fingerprints on Linux workers. The per-context patches are designed to make each context look like a *different person on the same OS*, not to impersonate a different OS entirely.
