...
This guide provides step-by-step instructions for updating Veyra patches when upgrading Firefox versions. Patches frequently break due to Firefox API changes, file reorganizations, and line number shifts.
...
All Veyra patches are in the `patches/` directory:
...
grep -r "FunctionName" veyra-<version>/ --include="*.cpp"
```
