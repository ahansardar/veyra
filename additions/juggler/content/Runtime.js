...
// Veyra: does this Runtime.callFunction carry a `mw:`-prefixed script?
//
...
// Veyra: report a bad `mw:` request the way an in-page exception is
...
// Veyra: the main-world half of the `mw:` escape hatch -- a standalone
...
class Runtime {
  ...
  async callFunction({executionContextId, functionDeclaration, args, returnByValue}) {
    ...
    // Veyra: page.evaluate() runs in an isolated world (see FrameTree.js), so
    ...
    if (isMainWorldRequest(functionDeclaration, args)) {
      ...
      ChromeUtils.veyraDebug(`Evaluating in main world: ${args[3].value}`);
  ...
  // Veyra: a main-world context is never registered in _executionContexts and
...
class ExecutionContext {
  constructor(runtime, domWindow, contextGlobal, auxData) {
    ...
    // Veyra: set by FrameTree on the default world when `allowMainWorld` is
  ...
  // Veyra: opt this world into the `mw:` escape hatch.
  //
  ...
  // Veyra: the context `mw:` scripts run in, built on first use. Building it
