...
// Veyra: Playwright's InitScript constructor wraps every body it is given:
//
//   (() => {
//         <source>
//       })();
...
class Frame {
  constructor(frameTree, runtime, browsingContext, parentFrame) {
    ...
    // Veyra: see _createIsolatedContext(). Held per frame because the master
  ...
  // Veyra: a "god mode" sandbox in its own compartment, running with the
  ...
  _createIsolatedContext(name, useMaster = false) {
    // Veyra: run the default world as the page's own world -- upstream
    ...
    // it is not a scraping mode. Veyra's own isolation is covered by
    // tests/patches/isolated-evaluate.py, which must keep running without it.
    if (!name && ChromeUtils.veyraGetBool('disableWorldIsolation', false)) {
    ...
    let sandbox;
    if (useMaster && ChromeUtils.veyraGetBool('forceScopeAccess', false)) {
    ...
    // Veyra: make the world's self-references mean the world's own global.
    //
    ...
    // Veyra: give the default world a main-world twin for the `mw:` escape
    ...
    if (!name && ChromeUtils.veyraGetBool('allowMainWorld', false))
  ...
  _onGlobalObjectCleared() {
    ...
    // Veyra: scope the default execution context to a sandbox so that
  ...
  // Veyra: run an init script in the world it asked for.
  //
  ...
  _evaluateInitScript(executionContext, script) {
    ...
    if (!ChromeUtils.veyraGetBool('allowMainWorld', false)) {
