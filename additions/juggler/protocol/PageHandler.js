...
export class PageHandler {
  constructor(target, session, contentChannel) {
    ...
    // Veyra: last position an actual mousemove landed on. Used as the
  ...
  async ['Page.reload']() {
    await this._pageTarget.activateAndRun(() => {
      ...
      // Veyra: Firefox 146's Browser:Reload command is a no-op on about:blank
  ...
  async ['Page.dispatchMouseEvent']({type, x, y, button, clickCount, modifiers, buttons}) {
    ...
    const sendEvents = async (types) => {
      ...
      for (const type of types) {
        // Veyra: when humanize is enabled, expand a direct mousemove into a
        // human-like trajectory of intermediate mousemoves generated in C++
        // (ChromeUtils.veyraGetMouseTrajectory / MouseTrajectories.hpp).
        if (type === 'mousemove' && ChromeUtils.veyraGetBool('humanize', false)) {
          const trajectory = ChromeUtils.veyraGetMouseTrajectory(this._lastTrackedPos.x, this._lastTrackedPos.y, x, y);
    ...
    await this._pageTarget.activateAndRun(async () => {
      ...
      if (type === 'mousemove') {
        ...
        // Veyra: remember where the cursor landed so the next humanized
