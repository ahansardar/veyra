/*
Helper to extract values from the VEYRA_CONFIG environment variable(s).
*/
...
inline const nlohmann::json& GetJson() {
  ...
  std::call_once(initFlag, []() {
    ...
    while (true) {
      std::string envName = "VEYRA_CONFIG_" + std::to_string(index);
    ...
    if (jsonString.empty()) {
      // Check for the original VEYRA_CONFIG as fallback
      auto originalConfig = get_env_utf8("VEYRA_CONFIG");
    ...
    if (!nlohmann::json::accept(jsonString)) {
      printf_stderr("ERROR: Invalid JSON passed to VEYRA_CONFIG!\n");
      jsonConfig = nlohmann::json{};
      return;
    }
