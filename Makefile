...
cf_source_dir := veyra-$(version)-$(release)
...
help:
	...
	@echo "  package-linux   - Package Veyra for Linux"
	@echo "  package-macos   - Package Veyra for macOS"
	@echo "  package-windows - Package Veyra for Windows"
	@echo "  run             - Run Veyra"
	@echo "  edit-cfg        - Edit veyra.cfg"
...
ff-dbg: setup
	...
	echo "LOCAL_INCLUDES += ['/veyracfg']" >> $(cf_source_dir)/dom/base/moz.build
...
run:
	cd $(cf_source_dir) \
	&& rm -rf ~/.veyra obj-x86_64-pc-linux-gnu/tmp/profile-default \
	&& VEYRA_CONFIG=$${VEYRA_CONFIG:-'{}'} \
	&& VEYRA_CONFIG="$${VEYRA_CONFIG%?}, \"debug\": true}" ./mach run $(args)

edit-cfg:
	@if [ ! -f $(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/veyra.cfg ]; then \
		echo "Error: veyra.cfg not found. Apply config.patch first."; \
		exit 1; \
	fi
	$(EDITOR) $(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/veyra.cfg
...
tests:
	cd ./tests && \
	bash run-tests.sh \
		--executable-path ../$(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/veyra-bin \
...
unbusy:
	rm -rf $(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/veyra-bin \
		$(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/veyra \
		$(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/launch

path:
	@realpath $(cf_source_dir)/obj-x86_64-pc-linux-gnu/dist/bin/veyra-bin
