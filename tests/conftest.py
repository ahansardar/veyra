...
def _run_in_the_pages_own_world() -> None:
    """Run this suite in the page's world rather than Veyra's isolated one.
    ...
    element handles into evaluate(). Veyra evaluates in an isolated world by
    ...
    for this suite alone. Veyra's isolated-world behaviour keeps its own
    ...
    this one.
    """
    raw = os.environ.get("VEYRA_CONFIG")
    veyra_config = json.loads(raw) if raw else {}
    veyra_config["disableWorldIsolation"] = True
    os.environ["VEYRA_CONFIG"] = json.dumps(veyra_config)
...
def launch_arguments(pytestconfig: pytest.Config, headless: bool) -> Dict:
    ...
    executable_path = os.getenv("VEYRA_EXECUTABLE_PATH", None)
    if executable_path:
