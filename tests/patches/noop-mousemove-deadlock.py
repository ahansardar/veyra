...
- Sync API. The bug only manifests through Playwright's *sync* client: the async
  ...
  redundant dispatch never reaches juggler. veyra's sync_api is a headline
...
Run against a specific build:
    VEYRA_EXECUTABLE_PATH=/path/to/veyra-bin python3 tests/patches/noop-mousemove-deadlock.py
