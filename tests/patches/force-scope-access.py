...
The flag had been declared in settings/properties.json and settings/veyracfg.jvv
...
Run against a specific build:
    VEYRA_EXECUTABLE_PATH=/path/to/veyra-bin python tests/patches/force-scope-access.py
(without the env var it uses the veyra-managed browser download.)
Against an unpackaged objdir build, run `make stage-fonts` first: these launch
through AsyncVeyra, which sets FONTCONFIG_FILE, and a build with no bundled
...
def _launch_kwargs(force_scope_access: bool) -> Dict[str, Any]:
    ...
    if force_scope_access:
        # Not a documented Veyra() kwarg; pass it straight through as config.
...
async def _run(force_scope_access: bool) -> bool:
    ...
    async with AsyncVeyra(**_launch_kwargs(force_scope_access)) as browser:
