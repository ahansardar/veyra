...
`ChromeUtils.veyraGetMouseTrajectory(...)` and dispatches the intermediate
...
Run against a specific build:
    VEYRA_EXECUTABLE_PATH=/path/to/veyra-bin python tests/patches/humanize-mouse-trajectory.py
...
async def _collect_moves(humanize):
    async with AsyncVeyra(**_launch_kwargs(humanize)) as browser:
...
async def _humanized_click_hits_target():
    async with AsyncVeyra(**_launch_kwargs(True)) as browser:
        page = await browser.new_page()
