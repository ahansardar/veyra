...
Run tests from dvsa-bot to get the right venv:
    ...
    uv run python ~/20tech/oss/veyra/tests/patches/<test>.py
"""
...
@asynccontextmanager
async def launch_veyra(
    os: str = "macos",
    headless: bool = True,
    max_attempts: int = MAX_PRESET_ATTEMPTS,
):
    """
    Launch Veyra with a random preset, retrying on WebGL mismatches.
    ...
    from veyra.fingerprints import generate_context_fingerprint, get_random_preset
