...
- Injecting JavaScript into the page or writing to DOM. Veyra's `page.evaluate` only supports reading values, not executing within the page context.
...
```bash
bash run-tests.sh --headful --executable-path /path/to/veyra-bin
```
